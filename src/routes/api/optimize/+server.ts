 ...
 * fetches candles once for the shared universe, then runs one backtest per
 * combination and returns the results ranked by the chosen metric.
 */

import { error, json } from '@sveltejs/kit';
import type { RequestHandler } from './$types';
import type { Candle, OptimizationSpec, OptimizeParam } from '$lib/types';
...
export const POST: RequestHandler = async ({ request, fetch }) => {
	...
	const { base, params, sortMetric } = body as {
		base?: unknown;
		params?: unknown;
		sortMetric?: unknown;
	};
	...
	return json(runOptimization(optSpec, candlesByTicker));
};
